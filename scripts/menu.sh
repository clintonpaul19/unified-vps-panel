...
while true; do
 ...
 case "$n" in
  ...
  6) echo 'Restarting backend services without dropping this SSH connection...'; systemctl restart xray hysteria-server unified-vps-panel; echo 'Backend services restarted.'; echo 'Do not restart SSLH from this menu while connected through ports 80/443/143/8080/8443, or your SSH session will be terminated.'; read -r -p 'Enter...' _ ;;
  7) exit 0 ;;
 esac
done
